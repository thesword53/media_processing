...
class ImageSplitter:
    ...
    def split_img_tensor(self, pil_img, scale_method=Image.BILINEAR, img_pad=0):
        ...
        if isinstance(pil_img, torch.Tensor):
            ...
            img_tensor = torch.from_numpy(pil_img)
        ...
        if scale_method is not None:
            ...
            img_up = torch.from_numpy(img_up)
    ...
    def merge_img_tensor(self, list_img_tensor, batch_size=1):
        out = torch.zeros((batch_size, 3, self.height * self.scale_factor, self.width * self.scale_factor))
